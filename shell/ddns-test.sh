...
install_ddns(){
    if [ ! -f "/usr/bin/ddns" ]; then
        curl -o /usr/bin/ddns https://raw.githubusercontent.com/mocchen/cssmeihua/mochen/shell/ddns-test.sh && chmod +x /usr/bin/ddns
    fi
    mkdir -p /etc/DDNS
...
这个脚本每一分钟都会推送ip更新到telegram，但我想让他在ip更换成新ip时在推送，怎么修改

ChatGPT
要实现在 IP 变更时才推送更新到 Telegram，你可以在脚本中添加逻辑来比较当前的公网 IP 与上一次记录的 IP 是否相同，只有在 IP 变更时才发送 Telegram 通知。以下是修改的建议：

首先，在 /etc/DDNS/.config 文件中添加两个变量来记录上一次的 IPv4 和 IPv6 地址：
bash
Copy code
Prev_IPv4=""
Prev_IPv6=""
在更新 DNS 记录之前，比较当前的 IPv4 和 IPv6 地址与上一次记录的地址是否相同。如果不同，则更新 DNS 记录，并更新上一次记录的地址，然后发送 Telegram 通知。

更新 /etc/DDNS/DDNS 脚本的一部分，添加 IP 变更检查和更新逻辑：
...
# 检查IP是否变更
if [[ "$Public_IPv4" != "$Prev_IPv4" || "$Public_IPv6" != "$Prev_IPv6" ]]; then
    # 更新IPv4 DNS记录
    curl -s -X PUT "https://api.cloudflare.com/client/v4/zones/$Zone_id/dns_records/$DNS_IDv4" \
         -H "X-Auth-Email: $Email" \
         -H "X-Auth-Key: $Api_key" \
    ...
    # 更新上一次记录的IP地址
    sed -i 's/^Prev_IPv4=".*"/Prev_IPv4="'"$Public_IPv4"'"/g' /etc/DDNS/.config
    sed -i 's/^Prev_IPv6=".*"/Prev_IPv6="'"$Public_IPv6"'"/g' /etc/DDNS/.config
